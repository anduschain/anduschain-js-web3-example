...
function compilingPreperations() {
    ...
    if (fs.existsSync(buildPath)) return new Error("build path is not empty, run npm run build:clean")
    return buildPath;
}
...
function writeOutput(compiled, buildPath) {
    ...
    for (let contractFileName in compiled.contracts) {
        ...
        fs.writeFileSync(
            ...
            JSON.stringify(compiled.contracts[contractFileName][contractName])
        );
    }
}
